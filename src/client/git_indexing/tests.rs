...
/// An isolated, temporary git repository with a controlled number of real commits, for tests
/// that need to reason about "N commits exist" or "more commits became available".
///
/// Depending on the ambient checkout's own history (e.g. via `path: "."`) is fragile: it silently
/// assumes that checkout always has enough real commits, which broke for real once this repo's
/// history got squashed down to a single commit. A repo built here always has exactly the number
/// of commits the test asked for, regardless of what state the actual checkout is in.
struct TempGitRepo {
    dir: TempDir,
}

impl TempGitRepo {
    /// Initializes a new repo and creates `count` commits, each adding one new file.
    fn with_commits(count: usize) -> Self {
        let dir = TempDir::new().unwrap();
        let repo = git2::Repository::init(dir.path()).expect("init temp git repo");
        ...
        for i in 0..count {
            ...
            std::fs::write(dir.path().join(&file_name), format!("content {i}"))
...
async fn test_search_git_history_incremental_indexing() {
    // Test that requesting more commits triggers incremental indexing. Uses an isolated temp
    // repo (5 real commits), not the ambient checkout ("."): this test needs to find *more*
    // commits on the second search than the first, and a repo built here always has exactly as
    // many commits as requested, regardless of the actual checkout's history.
    let (client, temp_dir) = create_test_client().await;
    let cache_path = temp_dir.path().join("git_cache.json");
    let git_repo = TempGitRepo::with_commits(5);
